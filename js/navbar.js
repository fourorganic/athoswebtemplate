...
(function () {
  ...
  var currentSlug = isProductPage ? path.replace(/^.*\//, '').replace(/\.html$/, '') : null;
  ...
  var items = PRODUCTS.map(function (p) {
    ...
    return '<li><a class="dropdown-item' + active + '" href="' + productHref(p.slug) + '">' + p.label + '</a></li>';
  }).join('');

  var html =
    '' +
    '<div class="container container-narrow py-2">' +
    '<a class="navbar-brand" href="' + home + '"><img src="' + logo + '" alt="Fourmidable" class="navbar-logo"></a>' +
    ...
    '<a class="nav-link dropdown-toggle' + dropdownActive + '" href="' + productos + '" id="productosDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">Productos</a>' +
    '<ul class="dropdown-menu dropdown-menu-end shadow-sm" aria-labelledby="productosDropdown">' +
    items +
    '<li><hr class="dropdown-divider"></li>' +
    '<li><a class="dropdown-item fw-semibold' + verTodosActive + '" href="' + productos + '">Ver todos los productos</a></li>' +
    '</ul>' +
    '</li>' +
